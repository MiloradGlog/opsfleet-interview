"""The analysis subgraph — sealed SQL lifecycle (brief R5: Resilience).

A compiled LangGraph ``StateGraph`` that owns the deterministic path:

    retrieve_trios -> generate_sql -> validate_sql -> execute_bq
                          ^                               |
                          +-------- repair_sql -----------+   (bounded by max_attempts)

* Retrieval is structurally first — the agent cannot skip it.
* generate_sql and repair_sql share one prompt site ("generate again, with the
  error in context"), so there is a single generation behavior.
...
* The loop is bounded by ``max_attempts`` and terminates into graceful_fail.

Collaborators (retrieval, LLM completion, query execution) are injected so the
subgraph is unit-testable offline with fakes.
"""
...
from .prompts import build_repair_prompt, build_sql_prompt
...
def build_analysis_subgraph(deps: AnalysisDeps):
    ...
    def repair_sql(state: AnalysisState) -> dict:
        ...
        return {"sql": _clean_sql(deps.complete(prompt)), "attempts": state.get("attempts", 1) + 1}
    ...
    def validate_sql(state: AnalysisState) -> dict:
        try:
            safe = sql_guard.validate(
                state.get("sql", ""), max_rows=deps.max_result_rows
            )
    ...
    def graceful_fail(state: AnalysisState) -> dict:
        if state.get("exec_result") == "empty":
            return {
                "status": "empty",
                "message": "No matching data was found for that question. Try broadening the "
                "time range or relaxing the filters.",
            }
        return {
            "status": "failed",
            "message": "I couldn't produce a valid query for that question after a few attempts. "
            "Try rephrasing it or making it more specific (for example, name the metric, "
