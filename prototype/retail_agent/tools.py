"""Manager-facing tools. Built as closures over an ``AgentRuntime`` so they share
the wired collaborators and the session's user/thread identity.
...
def _format_analysis(result: dict) -> str:
    ...
    if status != "ok":
        # graceful empty/failed message, already jargon-free
    ...
    # Compact markdown table of the preview rows.
    if rows:
        header = "| " + " | ".join(columns) + " |"
    ...
    return (
        f"Query returned {row_count} row(s). Retrieved examples: {', '.join(trio_ids) or 'none'}.\n"
        f"SQL used:\n{sql}\n\nResults:\n{table}{note}\n\n"
...
def build_tools(runtime) -> list:
    ...
    def list_reports() -> str:
        ...
        lines = [f"#{r['id']} — \"{r['title']}\" (saved {r['created_at'][:10]})" for r in rows]
