"""Static, business-language schema catalog for the four mandated tables.

Serves two purposes:
* :func:`describe_schema` answers schema-discovery questions in plain language
  with **no SQL execution** (brief "Expected Agent Capabilities").
* :func:`schema_for_prompt` produces a compact column listing that grounds SQL
  generation.

The catalog is static (the thelook_ecommerce schema is fixed), which keeps the CLI
startable and the unit tests runnable offline with zero BigQuery calls.
"""
...
_CATALOG: dict[str, tuple[str, list[tuple[str, str, str]]]] = {
    ...
    "users": (
        ...
        [
            ...
            ("email", "STRING", "Email address — personal data, excluded from analyses"),
...
# Columns that hold personal data. The agent does not query these (the SQL prompt
# steers away from contact columns); flagged here so describe_schema can say so.
...
def schema_for_prompt() -> str:
    ...
    lines = [
        f"Dataset: {DATASET_ID} (read-only). Reference tables fully-qualified, "
        f"e.g. {qualified('orders')}.",
        "",
    ]
...
def describe_schema(topic: str = "") -> str:
    ...
    Answers "what can I ask about" with NO SQL execution.
    """
