...
and the analysis subgraph from ``Settings`` over the shared Postgres pool.

Lazy construction means the CLI starts (and report-management commands work) even
before any analytical call, and a missing credential surfaces only when the feature
that needs it is used.
"""
...
def extract_text(resp: Any) -> str:
    """Extract plain text from a chat-model response whose ``.content`` may be a
    str OR a list of content blocks (langchain_google_genai 4.2.4 returns the
    latter, e.g. ``[{'type': 'text', 'text': '...'}]``)."""
...
class AgentRuntime:
    def __init__(self, settings: Settings, thread_id: str, pool=None) -> None:
        self.settings = settings
        self.thread_id = thread_id
        self.user_id = settings.user_id
        self.schema = schema_for_prompt()
        # ``pool`` is injectable for offline tests; otherwise open the real one.
        self.pool = pool if pool is not None else get_pool(
            settings.database_url, settings.embed_dim
        )
        self.storage = Storage(self.pool, settings.hmac_secret)
    ...
    def embed_fn(self, texts: list[str]) -> list[list[float]]:
        """Embed a batch of texts to fixed-dim vectors (truncated to embed_dim)."""
    ...
    def runner(self) -> BigQueryRunner:
        if self._runner is None:
            # project_id is the BILLING project (opsfleettest); creds come from
    ...
    def golden(self) -> GoldenBucket:
        if self._golden is None:
            self._golden = GoldenBucket(self.pool, self.embed_fn)
        return self._golden
    ...
    def _complete(self, prompt: str) -> str:
        ...
        return extract_text(self.llm.invoke(prompt))
